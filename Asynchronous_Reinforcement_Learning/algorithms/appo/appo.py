...
from Asynchronous_Reinforcement_Learning.algorithms.appo.model import create_actor_critic
from Asynchronous_Reinforcement_Learning.algorithms.algorithm import ReinforcementLearningAlgorithm
from Asynchronous_Reinforcement_Learning.algorithms.appo.actor_worker import ActorWorker
from Asynchronous_Reinforcement_Learning.algorithms.appo.appo_utils import TaskType,make_env_func, iterate_recursively, set_global_cuda_envvars
from Asynchronous_Reinforcement_Learning.algorithms.appo.learner import LearnerWorker
from Asynchronous_Reinforcement_Learning.algorithms.appo.policy_worker import PolicyWorker
from Asynchronous_Reinforcement_Learning.algorithms.appo.population_based_training import PopulationBasedTraining
from Asynchronous_Reinforcement_Learning.algorithms.appo.shared_buffers import SharedBuffers
from Asynchronous_Reinforcement_Learning.algorithms.utils.algo_utils import EXTRA_PER_POLICY_SUMMARIES, EXTRA_EPISODIC_STATS_PROCESSING, \
    ExperimentStatus
from Asynchronous_Reinforcement_Learning.envs.env_utils import get_default_reward_shaping
from Asynchronous_Reinforcement_Learning.utils.timing import Timing
from Asynchronous_Reinforcement_Learning.utils.utils import summaries_dir, experiment_dir, log, str2bool, memory_consumption_mb, cfg_file, \
    ensure_dir_exists, list_child_processes, kill_processes, AttrDict, done_filename, save_git_diff, init_file_logger
from Asynchronous_Reinforcement_Learning.utils.wandb_utils import init_wandb, finish_wandb

from typing import Any, Dict, Optional, Tuple, Union

if os.name == 'nt':
    from Asynchronous_Reinforcement_Learning.utils import Queue as MpQueue
else:
...
class CFG:
    def __init__(
        ...
        self.lr_schedule_kl_threshold = 0.008
        self.train_for_env_steps = 10000000000
        self.train_for_seconds = 10000000000
        ...
        self.policy_initialization = "orthogonal"
        self.policy_init_gain = 1.0
        self.actor_critic_share_weights = True
        self.use_spectral_norm = False
        self.adaptive_stddev = True
        self.initial_stddev = 1.0
        self.experiment_summaries_interval = 20
        self.adam_eps = 1e-06
        self.adam_beta1 = 0.9
        ...
        self.ppo_clip_value = 0.5
        self.batch_size = 1024
        self.num_batches_per_iteration = 1
        self.ppo_epochs = 1
        self.num_minibatches_to_accumulate = -1
        self.max_grad_norm = 0.5
        self.exploration_loss_coeff = 0.01
        self.value_loss_coeff = 0.5
        self.kl_loss_coeff = 0.0
        self.exploration_loss = "entropy"
        self.max_entropy_coeff = 0.0
        self.num_envs_per_worker = 2
        self.worker_num_splits = 2
        self.num_policies = 1
        self.policy_workers_per_policy = 1
        self.max_policy_lag = 10000
        self.traj_buffers_excess_ratio = 1.3
        self.decorrelate_experience_max_seconds = 10
        self.decorrelate_envs_on_one_worker = True
        self.with_vtrace = True
        self.vtrace_rho = 1.0
        self.vtrace_c = 1.0
        self.set_workers_cpu_affinity = True
        self.force_envs_single_thread = True
        self.reset_timeout_seconds = 120
        self.default_niceness = 0
        self.train_in_background_thread = True
        self.learner_main_loop_num_cores = 1
        self.actor_worker_gpus = []
        self.with_pbt = False
        self.pbt_mix_policies_in_one_env = True
        self.pbt_period_env_steps = 5000000
        self.pbt_start_mutation = 20000000
        self.pbt_replace_fraction = 0.3
        self.pbt_mutation_rate = 0.15
        self.pbt_replace_reward_gap = 0.1
        self.pbt_replace_reward_gap_absolute = 1e-06
        self.pbt_optimize_batch_size = False
        self.pbt_optimize_gamma = False
        self.pbt_target_objective = "true_reward"
        self.pbt_perturb_min = 1.05
        self.pbt_perturb_max = 1.5
        self.use_cpc = False
        self.cpc_forward_steps = 8
        self.cpc_time_subsample = 6
        self.cpc_forward_subsample = 2
        self.with_wandb = False
        self.wandb_user = None
        self.wandb_project = "Asynchronous_Reinforcement_Learning"
        self.wandb_group = None
        self.wandb_job_type = "SF"
        self.wandb_tags = []
        self.benchmark = False
        self.sampler_only = False
...
class APPO(ReinforcementLearningAlgorithm):
    ...
    def add_cli_args(cls, parser):
        ...
        arg('--wandb_project', default='Asynchronous_Reinforcement_Learning', type=str, help='WandB "Project"')
...
# set inital nn parameters
    ...
    def create_actor_worker(self, idx, actor_queue):
        ...
        return ActorWorker(
            ...
            report_queue=self.report_queue, learner_queues=learner_queues,rolloutoverqueue=self.rolloutoverqueue[idx]
        )
    ...
    def init_workers(self):
        ...
        for policy_id in range(self.cfg.num_policies):
            self.global_queues[policy_id]=TorchJoinableQueue()

        for actor_id in range(self.cfg.num_workers):
            self.rolloutoverqueue[actor_id]=TorchJoinableQueue()
    ...
    def _should_end_training(self):
        end = len(self.env_steps) > 0 and all(s > self.train_for_env_steps for s in self.env_steps.values())
            ...
            while not terminate:
                    for i in range(self.cfg.num_workers):
                        if not self.rolloutoverqueue[i].empty():
                            ...
                            if task_type == TaskType.ROLLOUT_OVER and self.terminateRolloutWorker.count(data) == 0:
                                self.terminateRolloutWorker.append(data)
                                self.actor_workers[data].close()
                                tmp_list.remove(data)
                                time.sleep(0.01)
                    if len(self.terminateRolloutWorker) == self.cfg.num_workers:
                        #time.sleep(0.01)
                        for workers in self.policy_workers.values():
                                all_workers.extend(workers)
                        all_workers.extend(self.learner_workers.values())
                        log.info('All actor worker finished')                     
                    elif time.time()-start >2:
                        for i, v in enumerate(tmp_list):
                          all_workers.extend(self.actor_workers[v])
                        for workers in self.policy_workers.values():
                            all_workers.extend(workers)
                        all_workers.extend(self.learner_workers.values())
                        log.info('All actor worker timeout') 
