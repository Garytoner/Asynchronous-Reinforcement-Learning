...
$ python -m Asynchronous_Reinforcement_Learning.runner.run --run=paper_doom_battle2_appo --runner=processes --max_parallel=4 --pause_between=10 --experiments_per_gpu=1 --num_gpus=4

Parallelize with Slurm:
$ python -m Asynchronous_Reinforcement_Learning.runner.run --run=megaverse_rl.runs.single_agent --runner=slurm --slurm_workdir=./megaverse_single_agent --experiment_suffix=slurm --pause_between=1 --slurm_gpus_per_job=1 --slurm_cpus_per_gpu=12 --slurm_sbatch_template=./megaverse_rl/slurm/sbatch_template.sh --slurm_print_only=False

Parallelize with NGC (https://ngc.nvidia.com/):
$ python -m Asynchronous_Reinforcement_Learning.runner.run --run=rlgpu.run_scripts.dexterous_manipulation --runner=ngc --ngc_job_template=run_scripts/ngc_job_16g_1gpu.template --ngc_print_only=False --train_dir=/workspace/train_dir
```
...
Arguments:
  ...
  --run RUN             Name of the python module that describes the run, e.g.
                        Asynchronous_Reinforcement_Learning.runner.runs.doom_battle_hybrid
  --runner {processes,slurm,ngc}
...
```
from Asynchronous_Reinforcement_Learning.runner.run_description import RunDescription, Experiment, ParamGrid
...
_experiments = [
    Experiment(
        'basic_envs_fs4',
        'python -m Asynchronous_Reinforcement_Learning.algorithms.appo.train_appo --train_for_env_steps=500000000 --algo=APPO --env_frameskip=4 --use_rnn=True --num_workers=36 --num_envs_per_worker=8 --num_policies=1 --ppo_epochs=1 --rollout=32 --recurrence=32 --batch_size=2048 --wide_aspect_ratio=False',
